...
GROUPS_DATA = [
    ...
    },
    {
        ...
        "description": "По суботах зібрання є відкритими, можуть приходити незалежні гості.",
        "schedule": {
            "0": "19:00-20:15", "1": "19:00-20:15", "2": "19:00-20:15",
...
def format_schedule(weekday_idx):
    """Формирует текст сообщения на основе данных о группах"""
    day_str = str(weekday_idx)
    
    # Среда — "Добрий день!", остальные — "Добрий ранок!"
    greeting = "🌞 Добрий день!" if weekday_idx == 2 else "🌞 Добрий ранок!"
    ...
    for item in GROUPS_DATA:
        ...
        if time:
            ...
            if item.get("description") == "Telegram":
                ...
                body += f"Адреса: {item['address']}\n"
                
            body += f"Час: {time}\n"
            
            # Дополнительное описание (если есть и это не "Telegram")
            if item.get("description") and item.get("description") != "Telegram":
                body += f"{item['description']}\n"
                
            # Как пройти (если есть)
            if item.get("directions"):
                body += f"Як пройти до нас: {item['directions']}\n"
